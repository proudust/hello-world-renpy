...
## init offset 文は、このファイルの init コードを他のファイルよりも先に実行して
## います。
init offset = -2
...
## メインメニューにゲームタイトルとそのバージョンを表示するかどうか。True なら表
## 示します。（この変数は options.rpy で再定義されるため、options.rpy の同じコー
## ドを消去しないと反映されません）
...
## 一般的なフレーム。デフォルトのインターフェースでは使われず、開発者の書いたコ
## ードでのみ使用します。画像は gui/frame.png。
