...
## displayable（表示可能オブジェクト）を必ず作成しなければなりません。また、ス
## タイルのプロパティを適用するために、ＩＤ "who" とＩＤ "window" を持つ text
## displayable も作成するといいでしょう。
##
...
## input displayable（表示可能オブジェクト）を作成する必要があります。
##
...
## Choice（選択）スクリーン #############################################################
##
## このスクリーンは、ゲーム内の選択肢を表示するのに使います。items のパラメータ
## ーは caption（選択肢のテキスト）と action（クリック時の実行内容）を要素に持つ
## オブジェクトのリスト（配列）です。
##
...
## 次が True の場合、menu の見出しテキストを narrator を使って say（発話）スクリ
## ーンで表示します。 False の場合、選択肢の上に押せないボタンとして表示します。
...
##
## クイックメニューはゲーム中つねに表示されるスクリーンで、ゲーム外の機能に素早
## くアクセスすることができます。
...
screen main_menu():
    ...
    ## use 文は、他のスクリーンを現在のスクリーンの内に表示するのに使います。メ
    ## インメニューの実際のコンテンツは navigation（ナビゲーション）スクリーンで
    ## す。
    use navigation
...
screen about():
    ...
    ## 次の use 文は game_menu（ゲームメニュー）スクリーンをこのスクリーンの内に
    ## 表示しています。use 文の子（内包されたオブジェクト）の vbox は game_menu
    ## スクリーンの中の viewport に配置されます。
...
screen history():
    ...
    use game_menu(_("ヒストリー"), scroll=("vpgrid" if gui.history_height else "viewport")):
